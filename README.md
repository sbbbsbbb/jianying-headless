...
口播粗剪（语音识别 → 选句子 → 带字幕的草稿）：

```bash
pip install mlx-whisper              # 本机语音识别，首次运行会下载约 1.6GB 的模型
mkdir -p work/lite/src               # 把要剪的 H.264 视频放进这里
python3 lite/auto_cut.py             # 列出识别出的句子，输入要保留的序号、标题、字幕替换词
python3 lite/export.py
```

选哪些句子由人或大模型决定，脚本只负责把决定变成时间线：主轨按句切开并首尾相接，字幕按词的时间分行，
同一份草稿可以再放一份到剪映草稿目录里继续手改。实测用一段 205 秒的 NASA 公有领域解说视频，保留 10 句，
得到 55 秒、10 段视频、24 条字幕的草稿；无界面导出 1652 / 1652 帧（1080p），
再对成片做一次语音识别，130 个词与应保留的文稿逐词一致。

这次实测踩到的两个坑，脚本里都已处理：

- **切点必须落在帧网格上，而且微秒要向下取整。** 引擎按时间线总时长向上取整出帧：
  总时长 53.949996 秒导出 1619 帧（按四舍五入应为 1618）；把 1652 帧四舍五入成 55066667 微秒，
  只多了不到 1 微秒，导出就成了 1653 帧。`frames_to_us` 用 `帧数 * 1000000 // 30`，`export.py` 的帧数校验同样按向上取整。
- **Whisper 的词起点常偏晚。** 句首只留 0.08 秒余量时，成片里一句开头的 “Radiation” 被切掉了辅音；留 0.2 秒后逐词核对全部一致。
